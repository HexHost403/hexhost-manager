...
banner() {
  ...
  echo
  border
  center "HEXHOST MANAGER"
  center "Premium Hosting Automation CLI"
  border
  echo
}
...
run_silent() {
  ...
  if [ "$status" -eq 0 ]; then
    ...
    echo -e "${YELLOW}${BOLD}Log:${RESET} $LOG_FILE"
    return 1
  fi
}
...
install_panel() {
  ...
  echo -e "${CYAN}${BOLD}Real logs:${RESET} $LOG_FILE"
  echo

  run_silent "Preparing server" "apt update -y && apt install -y software-properties-common curl apt-transport-https ca-certificates gnupg unzip tar git nginx redis-server mariadb-server" || { pause; return; }
...
SERVICE
  ...
  echo
  warn "Point DNS to this VPS, then install SSL."
  pause
}
...
kv() {
  printf "${RED}${BOLD}|${RESET} ${WHITE}${BOLD}%-12s${RESET} : ${CYAN}${BOLD}%-39.39s${RESET} ${RED}${BOLD}|${RESET}\n" "$1" "$2"
}

status_line() {
  printf "${RED}${BOLD}|${RESET} ${GREEN}${BOLD}%-8s${RESET} ${WHITE}${BOLD}%-45.45s${RESET} ${RED}${BOLD}|${RESET}\n" "OK" "$1 running"
}

warn_line() {
  printf "${RED}${BOLD}|${RESET} ${YELLOW}${BOLD}%-8s${RESET} ${WHITE}${BOLD}%-45.45s${RESET} ${RED}${BOLD}|${RESET}\n" "WARN" "$1 not running"
}
